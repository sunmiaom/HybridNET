...
for i in range(len(similarid_list)):
    for j in range(i + 1, len(similarid_list)):
        ...
        X = me.calculate_entropy_similarity(spectral_library_all[idx_i]["peaks"], spectral_library_all[idx_j]["peaks"],
...
similarities_df.to_csv(r'entropy.csv',index=False)
...
# Save figure
plt.savefig(r'entropy.png', dpi=600, bbox_inches='tight')
plt.show()
