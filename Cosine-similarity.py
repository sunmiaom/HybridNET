...
# Add a cosine similarity calculation function.
...
for parent in tqdm(parent_spectra):
    for product in product_spectra:
        similarity = calculate_cosine_similarity(parent["peaks"], product["peaks"], ms2_tolerance_in_da=0.01)
        if similarity > 0.5:
...
for i in range(len(similarid_list)):
    for j in range(i + 1, len(similarid_list)):
        ...
        X = calculate_cosine_similarity(spectral_library_all[idx_i]["peaks"],
...
similarities_df.to_csv(r"...cosine.csv",index=False)
...
# Save figure
plt.savefig(r'cosine.png', dpi=300, bbox_inches='tight')
plt.show()
